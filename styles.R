...
style_atlas <- function(textsize=7, family="Avenir Book", family.bold = "Avenir Heavy", is.cmyk = FALSE) {
  modifyList(style_base(textsize), listy(
    ...
    ## SHAPES & LINES ##########################################################
    theme = function() {
      # theme_minal() is a ggplot2 function
      theme_minimal() +
        # element_text(), element_blank(), element_rect(), rel(), and margin()  are also a ggplot2 function
        
        ##TODO:READ THIS
        ## Also read this documentation on 'Modify components of a theme' 
